<template>
  <div class="preview-panel">
    <div class="preview-content">
      <div class="presentation-container" ref="presentationContainer">
        ...
        <template v-else>
          ...
          <div v-for="(slide, slideIndex) in pptConfig.slides" :key="slideIndex" class="slide"
            :id="`slide_${slideIndex}`" @click="handleSlideClick(slideIndex)" :style="{
            ...
            <template v-if="slide.elements && Array.isArray(slide.elements)">
              <div v-for="(element, elementIndex) in slide.elements" :key="elementIndex"
                v-html="generateElementHTML(element)"
                :class="['element', { 'selected': isElementSelected(slideIndex, elementIndex) }]"
                @click.stop="handleElementClick(slideIndex, elementIndex)"></div>
            </template>
          </div>
        </template>
      </div>
    ...
    <div class="info-panel" v-if="pptConfig">
      ...
      <p><strong>文件名称:</strong> {{ pptConfig.file_name }}</p>
...
function generateElementHTML(element: Element): string {
  ...
  const isEmbeddedTable = ['msoEmbeddedOLEObject'].includes(element.element_type_name || '');
  ...
  // 应用Vue版本的样式属性
  if (!element.style?.font_size) styleString += `; font-size: ${element.style?.font_size}`;
  if (!element.style?.color) styleString += `; color: ${element.style?.color}`;
  if (!element.style?.background_color) styleString += `; background-color: ${element.style?.background_color}`;
  ...
  // 根据元素类型渲染
  const elementType = element.element_type_name;
  switch (elementType) {
    case 'msoTextBox':
    case 'msoAutoShape':
    case 'text':
    ...
    case 'chart':
      ...
      console.log('处理图表元素elementHTML', elementHTML)
      break;
    case 'msoPicture':
    case 'image':
